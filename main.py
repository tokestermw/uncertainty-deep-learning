...
class Model:
    def __init__(self, model_name, dropout_keep_prob=1.0):
        ...
        if self.model_name == "simple_linear":
            ...
            self.weight = ed.models.Normal(loc=self.weight_mu, scale=self.weight_sigma)

            self.bias_mu = tf.zeros(1)  # tf.get_variable("bias_mu", [1])  
            self.bias_sigma = tf.ones(1)  # fixed hyperparameters
            self.bias = ed.models.Normal(loc=self.bias_mu, scale=self.bias_sigma)

            # -- set model
            self.nn_mean = self.weight * self.x + self.bias
            self.nn_sigma = tf.ones(1)  # fixed hyperparameters
            self.nn = ed.models.Normal(loc=self.nn_mean, scale=self.nn_sigma)

            # -- set variational parameters
            self.qweight = ed.models.Normal(
                loc=tf.get_variable("qweight_mu", initializer=tf.random_normal([1])),
                scale=tf.nn.softplus(tf.get_variable("qweight_sigma", initializer=tf.random_normal([1]))))

            self.qbias = ed.models.Normal(
                loc=tf.get_variable("qbias_mu", initializer=tf.random_normal([1])),
                scale=tf.nn.softplus(tf.get_variable("qbias_sigma", initializer=tf.random_normal([1]))))
...
if __name__ == '__main__':
    ...
    >>> python main.py simple_linear
    >>> python main.py simple_linear .9
    >>> python main.py bayesian_simple_linear
    """
    args = sys.argv
